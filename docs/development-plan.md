...
## Phase 1: Monorepo セットアップ + コアエンジンプロトタイプ 🎯

**Goal:** 「カメラ映像をRust経由でfMP4化し、OPFSに保存する」最小限のプロトタイプを完成させる
...
**Deliverable:**
- ブラウザ単体で「録画→fMP4化→OPFS保存」が動作するデモ
...
| Phase | 成功指標 |
|-------|---------|
| Phase 1 | ローカルで10分の録画が成功し、OPFSに正常なfMP4が保存される |
